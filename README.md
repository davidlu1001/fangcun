...
- **质量保证**：96 项单元 + 10 项回归 + 40 例视觉基线，纹理种子可复现（每次调用独立 `np.random.Generator`，无全局状态），确定性逐字节验证
...
# 用自己的字源图片（跳过自动抓取，无网络请求）
python cli.py --text "然苏" \
    --user-glyph 然=./my_glyphs/ran.png \
    --user-glyph 苏=./my_glyphs/su.png \
    --shape oval --style baiwen

# 印谱拓片（白字黑底）需加极性提示
python cli.py --user-glyph 印=./rubbing.png --polarity white_on_black
```

### 用自己的字源（v1）

除了自动抓取，你也可以**上传自己的字源图片**。书法练习者、学者，或有特定碑帖参考的人尤其适用。

- **Web UI**：选择 "我自己上传字源" 模式，每字独立上传一张已裁剪、对比度清晰的单字图片。
- **CLI**：`--user-glyph CHAR=PATH` 可重复；字序按 `--text` 排列，未指定 `--text` 则按出现顺序拼接。
- **极性提示**：默认自动检测；印谱拓片请用 `--polarity white_on_black`（或 UI 选 "白字黑底"）。

跳过抓取后不再做同源同体检查，自行保证上传图片书体一致，否则会产生风格混杂的 "Frankenstein" 效果。
...
106 项测试覆盖单字提取、版面、渲染、纹理、字源选择、错误类型、并发、用户上传字源，外加 40 例视觉回归基线。
